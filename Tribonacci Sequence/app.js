function tribonacci(signature, n) {
  ...
  if (n > 2) {
    ...
    for (let i = 3; i < n; i++) {
      let next = array[i - 3] + array[i - 2] + array[i - 1];
      array.push(next);
    }
  } else {
    for (i = 0; i < n; i++) {
...
console.log(tribonacci([4, 8, 16], 1));
...
And, worse part of it, regrettably I won
't get to hear non-native Italian speakers trying to pronounce it :(
...
non-negative number; if n == 0, then return an empty array (except in C return NULL) 
